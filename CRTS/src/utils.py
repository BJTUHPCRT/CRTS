...
def read_data(input_path, debug=True):
    ...
    X = df.loc[:, [x for x in df.columns.tolist() if x != 'c_287']].to_numpy() * 100
    # X = df.loc[:, [x for x in df.columns.tolist() if x != 'c_6']].to_numpy() * 100
    # y = np.array(df.c_6) * 100
    y = np.array(df.c_287) * 100
